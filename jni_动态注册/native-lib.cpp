...
void onCreate(JNIEnv *env, jobject thiz,jobject bundle) {
    //  super.onCreate(savedInstanceState);--begin
    jclass bundle1 = env->FindClass("android/os/Bundle");
    jclass mainactive1 =env->GetObjectClass(thiz);
    jclass cls1 = env->GetSuperclass(mainactive1);
    jmethodID  oncreate_mid = env->GetMethodID(cls1,"onCreate","(Landroid/os/Bundle;)V");
    env->CallNonvirtualVoidMethod(thiz,cls1,oncreate_mid,bundle);
    __android_log_print(4,"test->jni","jni->%s","oncreata_jni_native calling");
    //  super.onCreate(savedInstanceState);--over
    //log.i("test_jni","woshijni");--begin
    jstring arg1 = env->NewStringUTF("test->jni");
    jstring arg2 = env->NewStringUTF("woshijni_log");
    jclass Logclass = env->FindClass("android/util/Log");
    jmethodID  logi = env->GetStaticMethodID(Logclass,"i","(Ljava/lang/String;Ljava/lang/String;)I");
    env->CallStaticIntMethod(Logclass,logi,arg1,arg2);
    //log.i("test_jni","woshijni");--over
//    public native String stringFromJNI();调用jni自己的函数
    jmethodID  stringFromJNI_java_jni = env->GetMethodID(mainactive1,"stringFromJNI","()Ljava/lang/String;");
    jstring string22 =static_cast<jstring>(env->CallObjectMethod(thiz,stringFromJNI_java_jni));
    const char*  string222 = env->GetStringUTFChars(string22, nullptr);
    __android_log_print(4,"test->jni","jni->%s",string222);
//      __android_log_print(4,"test->jni","string22->%s",string22);
...
Java_com_example_hook_1ndk_1so_MainActivity_stringFromJNI(
    ...
    jstring cppstring = env->NewStringUTF(hello.c_str());
//    const char* content1 = env->GetStringUTFChars(cppstring, nullptr);
    return cppstring;
...
void AAAA(JNIEnv *env, jobject thiz) {
    ...
    if (testobj!= nullptr){
        __android_log_print(4,"test->jni","lalalalalallalallala%s","newobject SUCESS");
    }



}
void cccc(JNIEnv *env, jobject thiz,jobject test) {
...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm,void* reserved){
    ...
    JNINativeMethod jniNativeMethod[]= {
            ...
            {"newobject","()V",(void*)AAAA},
            {"getnostaticobjectField","(Ljava/lang/Object;)V",(void*)cccc}
    };
    jclass testclass = env->FindClass("com/example/hook_ndk_so/MainActivity");
    env->RegisterNatives(testclass,jniNativeMethod,sizeof(jniNativeMethod)/sizeof(JNINativeMethod));
    pthread_t thread;
    //创建线程
